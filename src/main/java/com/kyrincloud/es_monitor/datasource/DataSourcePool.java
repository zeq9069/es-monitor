...
public final class DataSourcePool {
	...
	private Queue<Connection> pools;//连接池队列

	private PgDataSource pds;//数据源

	private AtomicInteger isUsed = new AtomicInteger(0);//已使用的数据库连接
	...
	private static final class Instance {
		private static final DataSourcePool p = new DataSourcePool(new PgDataSource(
				new PropertiesConfig().getDataSource()));
	}
